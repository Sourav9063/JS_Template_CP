...
function minFromLength(len, digits) {
  for (let currentLen = len; currentLen <= len + 1; currentLen++) {
    const value = minForLength(currentLen, digits);
    if (value !== null) return value;
  }
  return null;
}

function maxBelowLength(len, digits) {
  for (let currentLen = len; currentLen >= 1; currentLen--) {
    const value = maxForLength(currentLen, digits);
    if (value !== null) return value;
  }
  return null;
}
...
function nextGE(s, digits) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    return minFromLength(len + 1, digits);
  }
...
function prevLE(s, digits) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    return maxBelowLength(len - 1, digits);
  }
  return s;
}
